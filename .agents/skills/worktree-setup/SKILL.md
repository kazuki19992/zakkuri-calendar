...
3. Create a Codex branch from the latest remote development branch:

   ```bash
   git fetch origin develop
   git gtr new codex/<task-name> --from origin/develop --yes
   ```

4. Resolve and verify the new worktree:

   ```bash
   WORKTREE_PATH="$(git gtr go codex/<task-name>)"
