...
- `CalendarScreen`は`onAddEvent(date)`、`onCreateExactAt(date, startTime)`、`onEditEvent(id)`を受け取る。

- [x] **Step 1: 失敗するカレンダー操作テストを書く**
