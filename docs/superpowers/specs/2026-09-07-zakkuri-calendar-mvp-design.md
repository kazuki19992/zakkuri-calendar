...
初期表示は今日と明日を横2列で表示する2日ビューとし、月ビューへ同じ画面上で切り替えられるようにする。2日ビューは前後操作と横スワイプで基準日を1日ずつ移動し、横スワイプでは方向に沿ったアニメーションを行う。移動単位は実機評価後に変更できるよう、状態調整hookへ閉じ込める。

月グリッドと2日ビューはReact Nativeの標準コンポーネントで独自描画する。祝日判定にはメンテナンスされている既存ライブラリを使い、カレンダーUIライブラリには依存しない。MVPでは選択中のビューを永続化せず、v1で最後に開いたビューを復元する。
...
画面はNotionを参考にした白、チャコール、暖色系グレーを基調とし、高彩度の原色を主要面へ使わない。ライト・ダークのsemantic tokenを`src/constants/theme.ts`へ集約し、カレンダー、予定一覧、予定作成モーダルへ一貫して適用する。土曜と日曜・祝日は彩度を抑えた補助色を使い、選択状態、今日、祝日、予定有無は色だけでなく枠、下線、ラベル、読み上げ情報でも伝える。

具体的なtoken、2日・月ビュー構成、横スワイプの挙動は`docs/superpowers/specs/2026-09-09-two-day-calendar-ui-design.md`を正とする。
...
3. アプリシェルと月ビュー
4. ざっくり予定のクイック作成
5. 2日ビュー中心の独自カレンダーUIとテーマ
6. 2日ビューの時間軸とグラデーション表示
7. 週ビュー
8. 週・月単位のざっくり予定
9. 正確な時刻と終日予定
10. 編集、削除、精度変更
