...
export function MonthGrid({ days, onSelectDate, variant = 'month' }: MonthGridProps) {
    ...
    <View testID="month-calendar">
      ...
      {weeks.map((week, index) => (
        <View key={index} testID="month-calendar.week" style={styles.week}>
          {week.map((day) => <MonthDayCell key={day.date} day={day} variant={variant} onPress={onSelectDate} />)}
        </View>
      ))}
    </View>
  );
